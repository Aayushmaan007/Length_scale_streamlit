...
def calculate_synchrony_scale_for_station_for_visualization(
    base_station_id, correlation_matrix, coordinates,
    correlation_threshold=0.7, fraction_threshold=0.5,
    max_radius_km=4000, radius_step_km=25
):
    """
    Calculates the synchrony scale for a single station and returns data for visualization.
    """
    def calculate_distance_km(coord1, coord2):
        return geodesic((coord1['lat'], coord1['lon']), (coord2['lat'], coord2['lon'])).kilometers

    radii = list(range(radius_step_km, max_radius_km + radius_step_km, radius_step_km))
    fractions = []
    
    ref_coord = coordinates[base_station_id]
    
    for radius in radii:
        stations_in_radius = []
        for station_id, coord in coordinates.items():
            if station_id == base_station_id or station_id not in correlation_matrix.index:
                continue
            if calculate_distance_km(ref_coord, coord) <= radius:
                stations_in_radius.append(station_id)
        ...
        correlations = correlation_matrix.loc[base_station_id, stations_in_radius]
        above_threshold = (correlations.abs() >= correlation_threshold).sum()
        fractions.append(above_threshold / len(stations_in_radius))

    # Determine the synchrony scale
    synchrony_scale = 0
    for radius, fraction in zip(radii, fractions):
        if fraction >= fraction_threshold:
            synchrony_scale = radius
        else:
            break
    ...
    return {
        "radii": radii,
        "fractions": fractions,
        "synchrony_scale": synchrony_scale,
